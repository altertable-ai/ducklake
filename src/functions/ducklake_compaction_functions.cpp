...
// Logical Operator
//===--------------------------------------------------------------------===//
DuckLakeLogicalCompaction::DuckLakeLogicalCompaction(idx_t table_index, DuckLakeTableEntry &table,
                                                     vector<DuckLakeCompactionFileEntry> source_files_p,
                                                     string encryption_key_p, optional_idx partition_id,
                                                     vector<string> partition_values_p, optional_idx row_id_start,
                                                     CompactionType type)
...
string DuckLakeLogicalCompaction::GetExtensionName() const {
	return "ducklake";
}

vector<ColumnBinding> DuckLakeLogicalCompaction::GetColumnBindings() {
	vector<ColumnBinding> result;
	result.emplace_back(table_index, 0);
	return result;
}

void DuckLakeLogicalCompaction::ResolveTypes() {
